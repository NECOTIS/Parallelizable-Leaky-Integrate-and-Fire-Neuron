...
class Model(nn.Module):
    ...
    def train_model(self, train_loader, valid_loader, test_loader, device):
        ...
        for epoch in range(self.config.epochs):
            ...
            if  metric_valid > best_metric_val:#  and (self.config.model_type != 'snn_delays' or epoch >= self.config.final_epoch - 1):
                ...
                save_model(self, self.config, 'Best_ACC')
            ...
            if  loss_valid < best_loss_val:#  and (self.config.model_type != 'snn_delays' or epoch >= self.config.final_epoch - 1):
                ...
                save_model(self, self.config, 'Best_Loss')
        ...
        save_results(loss_epochs, metric_epochs, best_metric_val, self.config)
        save_model(self, self.config, 'Last')
        """
        if self.config.use_wandb:
            wandb.run.finish()
        """
    ...
    def eval_model(self, loader, device):
        ...
        torch.save(self.state_dict(), eventid + str(self.config.slurm_id) + '.pt')
        self.eval()
        with torch.no_grad():
            ...
            self.load_state_dict(torch.load(eventid + str(self.config.slurm_id) + '.pt'), strict=True)
            if os.path.exists(eventid + str(self.config.slurm_id) + '.pt'):
                os.remove(eventid + str(self.config.slurm_id) + '.pt')
            else:
                print(f"File '{eventid + str(self.config.slurm_id) + '.pt'}' does not exist.")

        return np.mean(loss_batch), np.mean(metric_batch)
    


def save_model(model, config, state):
    filename = f"outputs/{config.save_model_path.replace('REPL', state)}"
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    torch.save(model.state_dict(), filename)
    


def save_results(loss_epochs, metric_epochs, best_metric_val, config):
    """
    ...
    config_dict['surrogate_function'] = str(config_dict['surrogate_function'])
    outputs = {
    ...
    filename = output_dir+f"results_psn_{str(timestamp)}{'' if slurm_id is None else '_'+str(slurm_id)}.json"
    os.makedirs(os.path.dirname(filename), exist_ok=True)
