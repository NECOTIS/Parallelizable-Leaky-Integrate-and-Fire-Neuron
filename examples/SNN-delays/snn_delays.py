...
class SnnDelays(Model):
    ...
    def build_model(self):
        ...
        elif self.config.spiking_neuron_type == 'paralif': 
            spk_threshold = 1.
            threshold_multiple = False
            if "D" in self.config.spike_mode:
                spk_threshold = torch.FloatTensor(self.config.n_hidden_neurons).uniform_(1e-3, 1e-1).tolist() if threshold_multiple else 0.01
            elif "T" in self.config.spike_mode:
                spk_threshold = torch.FloatTensor(self.config.n_hidden_neurons).uniform_(0.5, 1.5).tolist() if threshold_multiple else 1.
            self.blocks[0][1].insert(0, ParaLIFWrapper(ParaLIF(self.config.n_hidden_neurons, self.config.spike_mode, recurrent=False, fire=True, spk_threshold=spk_threshold, 
                                 learn_threshold=self.config.th_learned, tau_mem=self.config.tau_mem, tau_syn=self.config.tau_syn, learn_tau=False, surrogate_mode="atan", surrogate_scale=5.)))
        ...
        for i in range(self.config.n_hidden_layers-1):
            ...
            if self.config.spiking_neuron_type == 'lif': 
                ...
                if "D" in self.config.spike_mode:
                    ...
                    spk_threshold = torch.FloatTensor(self.config.n_hidden_neurons).uniform_(0.5, 1.5).tolist() if threshold_multiple else 1.
                self.block[1].insert(0, ParaLIFWrapper(ParaLIF(self.config.n_hidden_neurons, self.config.spike_mode, recurrent=False, fire=True, spk_threshold=spk_threshold, 
                                     learn_threshold=self.config.th_learned, tau_mem=self.config.tau_mem, tau_syn=self.config.tau_syn, learn_tau=False, surrogate_mode="atan", surrogate_scale=5.)))
        ...
        if self.config.spiking_neuron_type == 'lif':
            ...
            threshold_multiple = False
            if "D" in self.config.spike_mode:
                spk_threshold = torch.FloatTensor(self.config.n_outputs).uniform_(1e-3, 1e-1).tolist() if threshold_multiple else 0.01
            elif "T" in self.config.spike_mode:
                spk_threshold = torch.FloatTensor(self.config.n_outputs).uniform_(0.5, 1.5).tolist() if threshold_multiple else 1.
            self.final_block.append([ParaLIFWrapper(ParaLIF(self.config.n_outputs, self.config.spike_mode, recurrent=False, fire=False, spk_threshold=spk_threshold, 
                                 learn_threshold=self.config.th_learned, tau_mem=self.config.tau_mem, tau_syn=self.config.tau_syn, learn_tau=False, surrogate_mode="atan", surrogate_scale=5.))])
    ...
    def forward(self, x):
        ...
        if self.config.spiking_neuron_type != 'heaviside':
            ...
            if self.config.loss != 'spike_count' and self.config.spiking_neuron_type != 'paralif':
